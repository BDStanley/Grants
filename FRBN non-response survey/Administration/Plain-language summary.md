...
1. **One survey, two contact protocols.** A Polish online panel agency will invite a large random sample to a ~10-minute survey on politics and society. Forty-two per cent of the invitees (chosen at random) get an insistent treatment: an invitation plus three reminders. The other fifty-eight per cent get a single invitation and nothing else. Roughly 2,000 people will complete the survey. Every completed interview is tagged with its protocol and with the reminder that produced it.
...
4. **Measure, then correct.** For each question we estimate how far the two protocols yield different answers, and report that estimate with an honest margin of error. Where the difference is large enough to demonstrate, we apply selection models to produce corrected estimates and check them against hard benchmarks such as official turnout and election results. Where it is small enough to rule out a meaningful problem, we can certify that standard weighting is adequate — a genuinely useful, reassuring finding. And where the pilot simply cannot tell, we say so: on a sample of this size some questions will fall between the two, which is a limit of a pilot rather than a failure of it.
...
- **For Polish survey research:** this delivers a cheap, reusable diagnostic — essentially, randomly varying how hard a survey pushes for responses, something agencies do anyway — that any Polish study can bolt on to find out which of its questions are distorted by nonresponse and by how much. The immediate beneficiary is PGSW, the flagship academic study of Polish elections. The pilot also produces the thing a full-scale study needs before it can be designed: an estimate of how large these distortions actually are in a Polish panel.
...
The one-sentence version: **instead of assuming the people who ignore our surveys think like the people who answer them, we run a small randomized experiment inside the survey that measures — question by question — how far that assumption holds, and repairs the numbers where it doesn't.**

*A note on what a pilot can deliver.* Before any fieldwork, we built the entire analysis and ran it on simulated data where the right answer was known. That exercise changed the design: it showed that the contact protocol we had planned would have produced too small a difference between the two groups to detect the effects we expect, so the insistent arm gained a third reminder and the statistical plan was rebuilt around fewer, better-targeted tests. It also set expectations honestly. A study of this size will establish how large the problem is and which questions are most affected; it will not settle every question it raises. That is the normal division of labour between a pilot and the full-scale study it is designed to justify.
